...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "DISMISS_TOAST": {
      ...
      return {
        ...state,
        toasts: state.toasts.map((toast) => (toast.id === toastId || toastId === undefined ? { ...toast } : toast))
      };
    }
    case "REMOVE_TOAST":
      return {
...
export const ToastAction = ({ altText, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { altText: string }) => (
  <button {...props} />
);
